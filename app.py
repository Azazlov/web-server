...
def create_app():
    ...
    # Production mode: minify HTML responses and serve bundled assets
    production_mode = config.production_mode
    app.config['PRODUCTION_MODE'] = production_mode
    ...
    def inject_utils():
        return dict(is_previewable=is_previewable, production_mode=production_mode)

    # Production mode: minify HTML responses
    if production_mode:
        try:
            from flask_minify import Minify
            Minify(app=app, html=True, js=True, cssless=True)
            print('  [production] HTML minification enabled via flask-minify')
        except ImportError:
            print('  [warning] flask-minify not installed, skipping HTML minification')
...
def build_if_production():
    """Build minified assets if in production mode."""
    if config.production_mode:
        print("Building minified static assets...")
        try:
            from build import build
            build()
        except ImportError:
            print("  [warning] build.py not found, skipping asset build")
